...
def train(args, train_loader, test_loader, val_loader, model):
    ...
    for epoch in range(args.train_epochs + 1):
        ...
        # else:
        train_error, train_loss, output = epoch_ce(
            args, train_loader, model, epoch, args.device, optimizer
        )

        if epoch % args.train_evaluate_rate == 0:
            test_error, test_loss, _ = epoch_ce(
                args, test_loader, model, args.device, None, None
            )
            if val_loader is not None:
                validation_error, validation_loss, _ = epoch_ce(
                    args, val_loader, model, args.device, None, None
                )
                print(
                    now(),
        ...
        if train_loss < args.train_threshold:
            print(
                f"Reached train threshold {args.train_threshold} (train_loss={train_loss})"
            )
            break
...
def data_extraction(args, dataset_loader, model):
    ...
    y = (
        torch.zeros(args.extraction_data_amount)
    ...
    for epoch in range(args.extraction_epochs):
        ...
        if epoch % args.extraction_evaluate_rate == 0:
            extraction_score = evaluate_extraction(
                args, epoch, kkt_loss, loss_verify, x, x0, y0, ds_mean
            )
            if epoch >= args.extraction_stop_threshold and extraction_score > 3300:
                print(
                    "Extraction Score is too low. Epoch:",
                    epoch,
                    "Score:",
                    extraction_score,
                )
        ...
        if (
            args.extract_save_results_every > 0
            and epoch % args.extract_save_results_every == 0
...
def setup_args(args):
    ...
    if args.pretrained_model_path:
        args.pretrained_model_path = os.path.join(
            models_dir, args.pretrained_model_path
        )
...
def main_reconstruct(args, train_loader):
    ...
    extraction_model = load_weights(
        extraction_model, args.pretrained_model_path, device=args.device
    )
...
def validate_settings_exists():
    ...
    raise FileNotFoundError(
        "You should create a 'settings.py' file with the contents of 'settings.deafult.py', "
